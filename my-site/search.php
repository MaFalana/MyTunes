<?php
	...
	if( isset($_POST['like']) )
	{
		...
		for ($i = 0; $i < count($json); $i++)
		{
			...
			//{				
				$newSong = $json[$i];
				//echo '<script> alert("hellohibhibhi")</script>';
				echo "<script> alert('hello '".$newSong->title.")</script>";
				$lib->likeASong($newSong);
			//}
		}
...
function process() 
{
  document.getElementByClass("result-Div").value = document.getElementById("name").innerHTML;
  return true;
}
</script>
</html>